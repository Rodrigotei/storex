<x-client_layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            @foreach(session('cart') as $index => $item)
                ...
                <div class="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-[30px] p-4 flex items-center gap-4 sm:gap-6 shadow-sm">
                    <div class="w-20 h-20 sm:w-24 sm:h-24 flex-none rounded-[20px] overflow-hidden bg-slate-100 dark:bg-gray-950">
                        <img src="{{ $item['image'] ? Storage::disk('s3')->url($item['image']) : 'img/default.png' }}" class="w-full h-full object-cover">
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                            ...
                                            @foreach($item['variations'] as $variation)
                                                <div class="flex items-center justify-between gap-3 text-xs bg-slate-100 dark:bg-gray-800 rounded-xl px-3 py-2">
                                                    ...
                                                    @if($variation['additional_price'] > 0)
                                                        <span class="font-bold text-emerald-600 dark:text-emerald-400 whitespace-nowrap">+ R$ {{ number_format($variation['additional_price'], 2, ',', '.') }}</span>
                                                    @endif
                                                </div>
                                            @endforeach
                                ...
                                @if(!empty($item['observation']))
                                    <div class="mt-3">
                                        <span class="text-xs font-semibold uppercase text-slate-400">Observação</span>
                                        <p class="text-sm text-slate-600 dark:text-gray-300 mt-1">{{ $item['observation'] }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                        <div class="mt-4 flex items-center justify-between">
                            <div class="flex flex-col">
                                <span class="text-xs uppercase font-bold tracking-wider text-slate-400">Unitário</span>
                                <span class="text-sm font-bold text-slate-800 dark:text-white">R$ {{ number_format($item['final_price'], 2, ',', '.') }}</span>
                            </div>
                            <div class="flex flex-col items-end">
                                <span class="text-xs uppercase font-bold tracking-wider text-slate-400">Total</span>
                                <span class="text-lg font-black text-[#004aad] dark:text-blue-400">R$ {{ number_format($itemTotal, 2, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-col items-end justify-between gap-4 self-stretch">
                        <form action="{{ route('client.cart.delete', ['tenant' => app('store')->slug]) }}" method="POST">
                            ...
                            <button type="submit" onclick="return confirm('Deseja remover do carrinho o produto: {{ $item['name'] }}?')" class="p-2 text-slate-300 hover:text-red-500 transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-1 12a2 2 0 01-2 2H8a2 2 0 01-2-2L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                            </button>
                        </form>
                        <div class="bg-slate-100 dark:bg-gray-800 rounded-2xl px-4 py-2 flex flex-col items-center">
                            <span class="text-xs uppercase font-bold tracking-wider text-slate-400">Qtd</span>
                            <span class="text-sm font-black text-slate-800 dark:text-white leading-none">{{ $item['qty'] }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-[40px] p-8 top-24 shadow-xl shadow-blue-500/5">
