<x-client_layout>
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12" 
        x-data="{ activeImg: '{{ $product->productImages->first() ? Storage::disk('s3')->url($product->productImages->first()->img) : 'img/default.png' }}', qty: 1 }">
        <div class="mb-8">
                ...
                @if($product->productImages->count() > 0)
                    <div class="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
                        @foreach($product->productImages as $image)
                            <button @click="activeImg = '{{ Storage::disk('s3')->url($image->img) }}'" class="relative flex-none w-24 h-24 rounded-[20px] overflow-hidden border-2 transition-all duration-300" :class="activeImg === '{{ asset('storage/' . $image->img) }}' ? 'border-[#004aad] ring-4 ring-blue-500/10' : 'border-transparent opacity-60 hover:opacity-100'">
                                <img src="{{ Storage::disk('s3')->url($image->img) }}" class="w-full h-full object-cover">
                            </button>
                        @endforeach
                    </div>
                @endif
