<x-dashboard_layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
            <h2 class="font-bold text-2xl text-slate-800 dark:text-white tracking-tight">Serviços</h2>
            ...
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-slate-50 dark:bg-gray-800/50 border-b border-slate-200 dark:border-gray-800">
                        <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest">Serviço</th>
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-gray-800/30 transition-colors group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col">
                                    <span class="text-sm font-semibold text-slate-700 dark:text-gray-200">{{ $service->name }}</span>
                                    <span class="text-[10px] text-slate-400 uppercase">ID: #{{ $service->id }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($service->promotional_price != null && $service->promotional_price > 0)
                                    <span class="text-sm font-bold text-red-500">R$ {{ number_format($service->promotional_price, 2, ',', '.') }}</span>
                                @else
                                    <span class="text-sm font-bold text-[#004aad] dark:text-blue-400">R$ {{ number_format($service->price, 2, ',', '.') }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('dashboard.services.edit', $service->id) }}" class="p-2 text-slate-400 hover:text-amber-500 transition-colors" title="Editar">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </a>
                                    <form action="{{ route('dashboard.services.destroy', $service->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Deseja excluir o produto: {{ $service->name }}?')" class="p-2 text-slate-400 hover:text-red-500 transition-colors" title="Excluir">
    ...
    <script>
        function editCategory(id) { window.location.href = `/dashboard/services/${id}/edit`; }
        setTimeout(() => {
